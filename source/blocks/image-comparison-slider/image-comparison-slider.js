document.addEventListener("DOMContentLoaded", function (e) {
	...
	if (slider) {
		...
		const changeWidth = (eventMove) => {
			const pageX = eventMove.pageX
				? eventMove.pageX
				: eventMove.touches[0].pageX;

			let newWidthImg =
				((slider.offsetWidth - (pageX - slider.getBoundingClientRect().left)) /
					slider.offsetWidth) *
				100;

			let newWidthRange =
				((pageX -
					sliderRange.getBoundingClientRect().left -
			...
			imgBefore.style.width = `${Math.min(
				Math.max(100 - newWidthImg, 0),
				100
			)}%`;
		};
